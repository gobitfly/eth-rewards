...
func CalculateStateRoot(
	...
	var err error
	state, err = ProcessSlotsUsingNextSlotCache(ctx, state, signed.Block().ParentRoot(), signed.Block().Slot(), income)
	if err != nil {
		return [32]byte{}, errors.Wrap(err, "could not process slots")
	}

	// Execute per block transition.
	state, err = ProcessBlockForStateRoot(ctx, state, signed, income)
	if err != nil {
...
func ProcessBlockNoVerifyAnySig(
	...
	blk := signed.Block()
	st, err := ProcessBlockForStateRoot(ctx, st, signed, income)
	if err != nil {
		return nil, nil, err
	}
...
func ProcessOperationsNoVerifyAttsSigs(
	...
	case version.Phase0:
		state, err = phase0Operations(ctx, state, signedBeaconBlock, income)
		if err != nil {
			return nil, err
		}
	case version.Altair, version.Bellatrix:
		state, err = altairOperations(ctx, state, signedBeaconBlock, income)
		if err != nil {
			return nil, err
		}
	default:
...
func ProcessBlockForStateRoot(
	...
	state, err = ProcessOperationsNoVerifyAttsSigs(ctx, state, signed, income)
	if err != nil {
		tracing.AnnotateError(span, err)
	...
	state, err = altair.ProcessSyncAggregate(ctx, state, sa, income)
	if err != nil {
...
func altairOperations(
	...
	st, err = b.ProcessAttesterSlashings(ctx, st, signedBeaconBlock.Block().Body().AttesterSlashings(), v.SlashValidator, income)
	if err != nil {
		return nil, errors.Wrap(err, "could not process altair attester slashing")
	}
	st, err = altair.ProcessAttestationsNoVerifySignature(ctx, st, signedBeaconBlock, income)
	if err != nil {
