...
func ExecuteStateTransition(
	...
	set, postState, err := ExecuteStateTransitionNoVerifyAnySig(ctx, state, signed, income)
	if err != nil {
...
func ProcessSlotsUsingNextSlotCache(
	...
	parentState, err = ProcessSlots(ctx, parentState, slot, income)
	if err != nil {
...
func ProcessSlots(ctx context.Context, state state.BeaconState, slot types.Slot,
	...
	for state.Slot() < slot {
		...
		if time.CanProcessEpoch(state) {
			logrus.Infof("processing epoch transition at slot %d", slot)
			...
			case version.Altair, version.Bellatrix:
				state, err = altair.ProcessEpoch(ctx, state, income)
				if err != nil {
					tracing.AnnotateError(span, err)
